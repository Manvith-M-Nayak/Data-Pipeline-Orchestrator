...
class CentralManager:
    ...
    def _log(
        ...
        print(
            f"[Manager {state.run_id[:8]}] {tag} [{state.phase}] {action}: {reason} → {outcome}"
        )
    ...
    def validate_plan(self, state: RunState) -> dict:
        ...
        for i, s in enumerate(stages):
            ...
            if stype == "notebook":
                ...
                if not s.get("source_dataset") and not s.get("source_container"):
                    warnings.append(
                        f"Stage '{s.get('name', i)}' (copy) missing 'source_dataset'"
                    )
        ...
        if ok:
            self._log(
                state,
                "VALIDATE",
                f"{len(stages)} stages, {len(warnings)} warnings",
                f"ok — {', '.join(warnings[:2]) or 'clean'}",
                "ok",
            )
        else:
            self._log(
    ...
    async def run_plan_assurance(self, state: RunState, schema: dict) -> dict:
        ...
        try:
            ...
            d = {
                ...
                "summary": "assurance skipped (error)",
                "tiers": {"failure": None},
            }
        ...
        if sem and sem.get("available"):
            if sem.get("flagged"):
                self._log(
                    ...
                    "ASSURE: Intent match",
                    sem["reasoning"],
                    "flagged — advisory, human may override",
                    "warn",
                )
            else:
                self._log(
                    state, "ASSURE: Intent match", "plan matches request", "ok", "ok"
                )
    ...
    # ────────────────────────────────────────────────────────────────────────
    # Phase 2c — Cost optimization (via Cost Optimization Agent)
    # ────────────────────────────────────────────────────────────────────────
    def optimize_cost(
        self,
        state: "RunState",
        ...
        """
        Calls the CostOptimizationAgent with the resource_plan, plan, and
        performance_prediction that are already on the RunState.

        Must be called AFTER predict_performance() in Phase 2.

        Stores result on state.cost_optimization and logs a summary.
    ...
    async def execute_with_retry(
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            ...
            try:
                ...
                self._log(
                    state,
                    f"EXECUTE attempt {attempt + 1} EXCEPTION",
                    last_error[:200],
                    "retry" if attempt < self.MAX_RETRIES else "abort",
                    "warn" if attempt < self.MAX_RETRIES else "error",
                )
    ...
    async def run_assurance(
        ...
        self._log(
            ...
            "ok"
            if checks["all_stages_completed"]
            else "incomplete — some stages skipped",
    ...
    async def execute_run(
        ...
        try:
            ...
            if pa.get("overall_status") != "pass":
                failing = [
                    c["label"]
                    for c in pa.get("structural_results", [])
                    if not c["passed"]
                ]
                state.status = "failed"
                state.error = "Assurance Agent rejected plan — " + (
                    pa.get("tiers", {}).get("failure")
                    or ("failed: " + ", ".join(failing))
                )
            ...
            self._enter(
                state, "pre_checks", "Running resource prediction and cost estimation"
            )

...
def main():
    ...
    # Validate the path immediately — catch typos before anything hits Azure
    ...
    print("\n--- Step 2: Groq AI is deciding everything ---")
    pipeline_config = decide_pipeline_config(schema, user_prompt)

    # Show plan and confirm before touching Azure
    ...
    # ── Step 3b: Purge bronze and silver containers ──────────
    # Stale blobs from previous runs (*.csv wildcards, .txt files,
    # part-XXXXX partition files) pollute reads and produce unexpected
    # output. Wipe them clean before every run.
    ...
    # Verify the upload actually has data
    ...
    for p in pipeline_config["pipelines"]:
        if p["type"] == "copy":
            ...
            # Inject inferred_types so build_dataflow_script can declare
            # the source schema — without this ADF shows "Columns: 0 total"
            p["inferred_types"] = schema["inferred_types"]
            r = create_dataflow_pipeline(token, p, schema["columns"])
        else:
            print(f"   ⚠️  Unknown pipeline type '{p['type']}' — skipping")
            continue
    ...
    # All REST API changes sit in the ADF draft layer until published.
    # The executor ONLY runs the published (live) layer.
    ...
    for pipeline_name in pipeline_config["execution_order"]:
        ...
        if result["status"] != "Succeeded":
            print(f"\n❌ Pipeline '{pipeline_name}' did not succeed "
                  f"(status: {result['status']}). Stopping.")
            all_succeeded = False
            break

        # After the copy pipeline completes, verify the destination
        # container actually received rows before running the dataflow.
        # An empty sink from the copy step causes a confusing executor
        # error in the dataflow ("Dataflow cannot be analyzed as a graph").
        copy_pipeline_names = [
            p["name"] for p in pipeline_config["pipelines"] if p["type"] == "copy"
        ]
        if pipeline_name in copy_pipeline_names:
            # Find which container this copy pipeline writes to
            copy_cfg = next(
            ...
            if sink_ds:
                ...
                if not check_blob_has_rows(sink_container):
                    print(
                        f"❌ Copy pipeline '{pipeline_name}' wrote nothing to "
                        f"'{sink_container}'. "
    ...
    # Always save config for debugging
    ...
    if all_succeeded:
        ...
        print("\n⚠️  Pipeline run incomplete. Check the errors above.\n")

...
def preview_config(pipeline_config: dict, show_editable: bool = True):
    ...
    for p in pipeline_config["pipelines"]:
        ...
        if p["type"] == "dataflow":
            ...
            if dropped:
                print(f"    ⚠️  {len(dropped)} transform(s) flagged for self-healing:")
                for d in dropped:
                    print(f"      ✗ {d.get('transform', d)}")
            print(
...
# ============================================================
# SELF-HEALING: Redeploy a single pipeline after fix
# ============================================================
def _redeploy_pipeline(token: str, p: dict, schema: dict):
    """Redeploy a single pipeline (and its dataflow) after healing."""
    if p["type"] == "copy":
        ...
        print(f"   Unknown type '{p['type']}' — skipping redeploy")
        return False

    if r is None or r.status_code not in [200, 201]:
        print(f"   ❌ Redeploy failed for '{p['name']}'")
        return False

    print(f"   ✅ Redeployed '{p['name']}' successfully")
    return True


# ============================================================
# HELPER: Check if any pipeline has planner-dropped transforms
# ============================================================
def _has_dropped_transforms(pipeline_config: dict) -> bool:
...
def _describe_dropped_transforms(pipeline_config: dict) -> str:
    """Build a human-readable summary of what the planner couldn't handle."""
    lines = []
    for p in pipeline_config.get("pipelines", []):
        dropped = p.get("_dropped_transforms", [])
        for d in dropped:
...
def main():
    ...
    if _has_dropped_transforms(pipeline_config):
        summary = _describe_dropped_transforms(pipeline_config)
        ...
        if not healed:
            print("❌ Self-healing could not fix the config — aborting")
            return

        print("\n✅ Config healed. Updated pipeline plan:")
    ...
    raw_container = pipeline_config["containers"].get("stage0") or pipeline_config["containers"].get("raw") or list(pipeline_config["containers"].values())[0]
    pipeline_config["_csv_path"] = csv_filepath
    ...
    for pipeline_name in pipeline_config["execution_order"]:
        ...
        if result["status"] != "Succeeded":
            error_msg = result.get("error", "Unknown pipeline failure")
            ...
            if not healed:
                print("❌ Self-healing could not fix the issue — aborting")
                all_succeeded = False
                break

            pipeline_config = fixed_config

            print(f"\n🔁 Redeploying fixed pipeline: {pipeline_name}")
            failed_pipeline_cfg = next(
                (p for p in pipeline_config["pipelines"] if p["name"] == pipeline_name),
                None
            )
            if failed_pipeline_cfg:
                redeployed = _redeploy_pipeline(token, failed_pipeline_cfg, schema)
                if not redeployed:
                    print("❌ Redeploy failed — aborting")
                    all_succeeded = False
                    break

            print(f"\n   Waiting 15s for ADF to pick up the redeployed pipeline...")
            import time
            time.sleep(15)

            print(f"\n▶ Re-triggering healed pipeline: {pipeline_name}")
            retry_run_id = trigger_pipeline(token, pipeline_name)
            if not retry_run_id:
                print(f"❌ Could not re-trigger '{pipeline_name}' after healing — aborting")
                all_succeeded = False
                break

            retry_result = check_pipeline_status(token, pipeline_name, retry_run_id)

            if retry_result["status"] == "Succeeded":
                print(f"\n✅ '{pipeline_name}' SUCCEEDED after self-healing! 🎉")
            else:
                print(f"\n❌ '{pipeline_name}' still failing after healing — aborting")
                print(f"   Error: {retry_result.get('error', 'unknown')}")
                all_succeeded = False
                break

        else:
            print(f"✅ '{pipeline_name}' succeeded on first run")
